...
export class Telemetry {
    ...
    intialize(userId: string, deviceId: string, locale: string, permissions: string[]) {
    ...
    private observeNode(node: Element): void {
        if (node.nodeName !== 'A' &&
            ...
            node.getAttribute('data-product-analytics') === null
        ) {
            return;
        }
    ...
    private dispatchEvent<N extends EventTypes>(eventType: N, eventData: EventPayload<N>): void {
        if (!this.isInitialized) {
            // TODO implement queue to store events before telemetry object is initialized
        }
