...
final class ProductAdminSearchIndexer extends AbstractAdminIndexer
{
    ...
    public function globalCriteria(string $term, Search $criteria): Search
    {
        ...
        // The `completion` and `completion.ngram` clauses are added centrally
        // in `AdminSearcher::buildSearch` for every indexer. The product
        // indexer additionally boosts prefix matches on `textBoosted`, which
        // carries productNumber + customSearchKeywords + the concatenated
        // names — a strong signal when the user types the start of an
