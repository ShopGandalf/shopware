...
namespace Shopware\Tests\Unit\Core\Checkout\DocumentV2\Zugferd\Calculation;
...
class NetAmountTest extends TestCase
{
    public function testIsOrderGrossTrueForGrossCart(): void
    {
        ...
        $order->setPrice($this->cartPrice(CartPrice::TAX_STATE_GROSS));
    ...
    {
        $order = new OrderEntity();
        $order->setPrice($this->cartPrice(CartPrice::TAX_STATE_NET));
    ...
    {
        $order = new OrderEntity();
        $order->setPrice($this->cartPrice(CartPrice::TAX_STATE_FREE));
    ...
    private function cartPrice(string $taxState): CartPrice
    {
